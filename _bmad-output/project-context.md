...
- **Standard harness only:** inline `#[cfg(test)] mod tests` in the file under test, run by `cargo test` (no `tests/` directory). No test dev-dependencies (no mockall/proptest/rstest); the mock transport (`MockTransport` in `src/robot.rs` tests) is hand-written — scripted responses released per write, implementing the `Transport` trait. Property-style coverage is done with hand-rolled **deterministic** parameter sweeps, not `proptest` (see `every_pot_of_a_validated_plate_is_within_limits`, issue #21): assert the invariant over the whole grid and assert the sweep hits both accepted and rejected cases so it is not vacuous. (`unwrap()` in tests: see Rust rules.)
