...
- **`DeltaRobot` is owned exclusively by the worker thread** (`spawn_robot_worker`, `src/main.rs`). Never wrap it in `Arc<Mutex<…>>`, never call it from the UI thread. A new robot operation = a new `RobotCommand` variant, handled sequentially in the worker loop. Sole exception: the `SeedingControl` atomic flags — and their reset belongs to the worker at dequeue, never to the UI.
...
- **Decision logic is exposed as pure functions before the anyhow wrapping** (limit checks, pot-position math) so it is testable without hardware. Sequencing logic (protocol handshakes, ok-waiting, seeding loop) that needs tests requires a transport trait seam — extract one; never hack around the concrete `SerialCommunication`.
...
- **A UI callback body is one `tx.send(RobotCommand::…)` and nothing else** — no robot I/O, no waiting, no sleeps in callbacks. Sole exception: stop/pause/continue set the `SeedingControl` atomic flags directly.
...
- **Displayed values are formatted Rust-side** (`format!("{:.3}", x)`) and exposed as `string` properties — don't convert to numeric properties or move formatting into `.slint`. Lists: `[string]` ↔ `Rc<VecModel<SharedString>>`.
...
- **Touch targets:** existing buttons are 60px tall (160×60 main screen, 70×60 seeding) — match that; ≥60px for primary/destructive actions (gloved operator, ~133 ppi). Never place Stop adjacent to Pause/Continue without spacing. No hover states, no keyboard shortcuts, no scroll-only access to critical actions.
- **Strings:** `.slint` labels use `@tr("…")`. Strings pushed from Rust (status texts, formatted values) are raw English literals — known debt, assumed. Don't grow it needlessly; don't invent a Rust-side translation mechanism as a drive-by fix.
...
- **Characterization before change:** before modifying untested code (G-code generation, handshake), first write tests locking in the current correct behavior. The first protocol change pays for the transport trait seam (see Rust rules): extract it in that same change, script the current exchange in a mock, then alter behavior.
- **Standard harness only:** inline `#[cfg(test)] mod tests` in the file under test, run by `cargo test` (no `tests/` directory). No test dev-dependencies (no mockall/proptest/rstest); the mock transport is hand-written — scripted responses in a `Vec`, implementing the transport trait. (`unwrap()` in tests: see Rust rules.)
